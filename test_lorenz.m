%==========================================================================
% test the Lorenz data with our optimization model
...
%
% Visualizations:
%    1. Plot Lorenz system with outliers
...
params_data.ratio_corrupted = 0.008; % corruption ratio
...
params_alg.maxit = 100; % max number of iterations, default = 100
params_alg.coef_thres = 0.1; % thresholding parameter for the coefficient (coeffs - hard thresholding)
...
rho = 28.0;% 
beta = 8.0/3.0;
U0 = [-8.0 7.0 27.0]; % Starting point

% Generate clean data
myfunc = @(t,x) [sigma*(x(2)-x(1)); x(1).*(rho-x(3))-x(2); x(1).*x(2)-beta*x(3)];
...
figure('name','Lorenz with corrupted data');
...
a0 = [0, -sigma, sigma, 0,0,0,0]';%[1, x, y, z, x^2, xy, xz]
b0 =[0, rho, -1, 0,0,0,-1]';
c0 =[0, 0,0,-beta, 0,1,0]';
coeff_true = [a0 b0 c0];
