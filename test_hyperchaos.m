%==========================================================================
% test the Rossler HyperChaos data with our optimization model
%    x' = sigma*(y-x); y' = x*rho - xz - y; z' = xy - beta*z
%
% Inputs:
...
%
% Visualizations:
%    1. Plot Rossler HyperChaos system with outliers
...
params_alg.tol = 5e-3; % tolerance for relative change of the outliers, default = 5e-3
params_alg.maxit = 200; % max number of iterations, default = 100
params_alg.coef_thres = 0.01; % thresholding parameter for the coefficient (coeffs - hard thresholding)
params_alg.mu = 0.0125; % weight of the term ||phiX*C + E - Xdot + b||_F^2

% Parameters of the Rossler HyperChaos system
U0 = [-20.0 0.0 0.0 15.0]; % Starting point from Rossler's paper

% Generate clean data
myfunc = @(t,x) [-x(2)-x(3); x(1) + 0.25 * x(2) + x(4); 3 + x(1).*x(3); -0.5*x(3)+0.05*x(4)];
[T U_clean] = ode45(myfunc,[0:params1.dt:params1.Tfinal],U0,odeset('RelTol',1e-12,'AbsTol',[1e-12,1e-12,1e-12 1e-12]));
...
figure('name','Rossler 4D Hyperchaos XYW');
plot3(U(:,1), U(:,2), U(:,4),'.');%,'Linewidth',1.25); 
xlabel('x_1','FontSize',18)
ylabel('x_2','FontSize',18)
zlabel('x_4','FontSize',18)
set(gca,'FontSize',18)
axis tight;grid on; view(27,16)
figure;
plot(U(:,3),'LineWidth',1.8);set(gca,'FontSize',18); axis tight;


% Compare the recovered vs the true coefficients
% True Solution for the coefficient C

a0 = [0, 0, -1, -1,0,0,0,0]';%[1, x, y, z, w, x^2, xy,xz]
b0 = [0, 1, 0.25, 0,1,0,0,0]';
c0 = [3.0,0,0,0, 0, 0,0,1]';
d0 =[0,0,0,-0.5,0.05,0,0,0]';

coeff_ode_true = [a0 b0 c0 d0];
C_true= [coeff_ode_true; zeros(size(phiX,2)-8,4)];


figure('name','True and Recovered Coefficients')
    subplot(411); plot(C(:,1),'r*'); hold on; plot(C_true(:,1),'+b');
    legend('Numerical Soln','Ground-truth Soln')
    subplot(412); plot(C(:,2),'r*');hold on; plot(C_true(:,2),'+b');
    subplot(413); plot(C(:,3),'r*');hold on; plot(C_true(:,3),'+b');
    subplot(414); plot(C(:,4),'r*');hold on; plot(C_true(:,4),'+b');
